...
class FreeBusy {
  ...
  analyzeGroupAvailability(
    ...
    // Sort: Time ASC, then Type DESC (Starts [1] first)
    points.sort((a, b) =>
      a.time !== b.time ? a.time - b.time : b.type - a.type
    );

    // Group by timestamp to handle simultaneous changes
  ...
  async getAllUserCalendarLists() {
    ...
    try {
      ...
      if (result.calendars.length < this._minUsers) {
        result.error = new BadRequestError(
  ...
  async getBusyIntervals(calendarList) {
    ...
    if (this._validUsers.length < this._minUsers) {
      result.error = new BadRequestError(
    ...
    try {
      ...
      if (!allBusyIntervals.total) {
        /*
         ...
         Just return a large block and a flag to let the UI know what happened
        */
        return {
          blocks: [
            {
              ...
              availablePlayers: this._validUsers.length,
              availablePlayerList: this._validUsers,
              unavailablePlayerList: this._disconnectedUsers,
              totalUsers,
              duration:
                (new Date(this._startDate) - new Date(this._endDate)) / 60000,
            },
          ],
